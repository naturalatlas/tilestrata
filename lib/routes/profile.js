...
var field_order = ['errors', 'hits', 'misses', 'dur_min', 'dur_avg', 'dur_max', 'dur_samples', 'size_min', 'size_avg', 'size_max', 'size_samples'];
	...
	if (/format=json/.test(req.url)) {
		...
		fs.readFile(cssfile, 'utf8', function(err, css) {
			...
			layers.forEach(function(layer) {
				var escaped_layer = _.escape(layer);
				html.push('<h2><a href="#' + escaped_layer + '" name="' + escaped_layer + '">'+escaped_layer+'</a></h2>');
				var routes = Object.keys(data[layer]);
				routes.sort();
				html.push('<table cellspacing="0" cellpadding="0" border="1">');
				routes.forEach(function(route) {
					var escaped_route = _.escape(route);
					var route_hash = (escaped_layer + '/' + escaped_route).replace(/@/g, '_');
					html.push('<tr class="route-heading"><th colspan="2"><a href="#' + route_hash + '" name="' + route_hash + '"><em>'+escaped_route+'</em></a></th>');
					...
					plugins.forEach(function(plugin) {
						...
						fields_available.sort(function(a, b) {
						...
						fields_available.forEach(function(field, i) {
							...
							zooms.forEach(function(z) {
								...
								catch(e) { html.push('<td class="empty"></td>'); }
							});
							html.push('</tr>');
						});
					});
