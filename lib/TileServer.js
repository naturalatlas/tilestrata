...
/** @type {string} */
TileServer.prototype.version = require('../package.json').version;
 ...
 * @return {void}
 */
TileServer.prototype.serve = function(method, url, headers, hooks, callback) {
	...
	async.series([
		...
		function handleRequest(callback) {
			handler[_method](self, req, function(status, buffer, headers) {
				...
				headers['X-Powered-By'] = 'TileStrata/' + self.version;

				if (status === 200) {
					headers['Cache-Control'] = 'max-age=60';
					headers['ETag'] = etag(buffer);
					if (reqheaders && reqheaders['If-None-Match'] && reqheaders['If-None-Match'] === headers['ETag']) {
						status = 304;
						buffer = new Buffer([]);
					}
				}

				if (method === 'HEAD') buffer = new Buffer([]);
		...
		if (err) {
			callback(500, String(err.message || err), {});
		} else {
...
TileServer.prototype.respond = function(req, res) {
	var handler = this.serve(req.method, req.url, req.headers, {req: req, res: res}, function(status, buffer, headers) {
...
TileServer.prototype.getTile = function(layer, filename, x, y, z, callback) {
	...
	this.serve('GET', req, {}, false, function(status, buffer, headers) {
		if (status === 200) {
