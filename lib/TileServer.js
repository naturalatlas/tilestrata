...
TileServer.prototype.checkHealth = function(callback) {
	if (!this.healthy) return callback();
	this.healthy(callback);
};
...
TileServer.prototype.listen = function(port, callback) {
	...
	var server = http.createServer(function(req, res) {
		if (req.url === '/health') {
			return self.checkHealth(function(err, result) {
				var status = 200;
				var host = process.env.TILESTRATA_HIDEHOSTNAME ? '(hidden)' : hostname;
				var data = _.extend({ok: true, version: version, host: host}, result);
				if (err) {
					status = 500;
					data.ok = false;
