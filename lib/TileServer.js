...
TileServer.prototype.bindBalancer = function() {
	...
	var recovery = this.balancer = new Recovery({
		'retries': Infinity,
		'min': balancer.register_mindelay || '100 ms',
	...
	recovery.on('reconnect', function(opts) {
		...
		request({
			json: true,
			method: 'POST',
			url: 'http://' + balancer.host + '/register',
			...
			log.info('balancer', 'Successfully registered with ' + res.connection.remoteAddress);
...
TileServer.prototype.close = function(callback) {
	...
	if (!this.http_server) return callback();
